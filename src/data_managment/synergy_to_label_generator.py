...
import random
from src.utils.cards_advisor import load_embeddings_cards, recommend_cards
...
def generate_synergy_labels(
    ...
    for _ in tqdm(range(card_to_do), desc="Generating synergy labels", unit="card"):
        ...
        top_synergies = recommend_cards(
            [card_name], filtered_embeddings, synergy_model, top_n=synergies_per_card
        )
...
if __name__ == "__main__":
    ...
    synergies_hashmap = {
        f"{synergy['card1']}{synergy['card2']}": synergy for synergy in synergy_labels
    }

    # Load synergy model
    synergy_model = build_synergy_model(
        arch_name=SYNERGY_MODEL_ARCH,
        embedding_dim=EMBEDDING_DIM,
        tag_projector_dim=EMBEDDING_DIM,
        initialize_weights=False,
    )
    synergy_model.load_state_dict(
        torch.load(SYNERGY_CHECKPOINT_FILE, map_location=DEVICE)
    )
