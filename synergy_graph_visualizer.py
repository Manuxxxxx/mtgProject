...
def load_or_compute_umap(all_cards, card_names, umap_file_path="umap_coords.npy", n_components=2):
    if os.path.exists(umap_file_path):
        ...
        #print the first 100 coordinates
        print("First 100 UMAP coordinates:")
        for i in range(min(100, len(coords))):
            print(f"{i:3d}: {coords[i]}")
        # Save the computed UMAP coordinates
        np.save(umap_file_path, coords)
        print(f"UMAP coords computed and saved to {umap_file_path}")
    return coords

def extract_all_sets(all_cards):
    sets = set()
    for _, card in all_cards.items():
...
EDGE_LIMIT = 2000  # max edges to send for performance
...
all_sets = extract_all_sets(all_cards)

umap_coords = load_or_compute_umap(all_cards, card_names, umap_file_path=UMAP_FILE, n_components=2)

conn = sqlite3.connect(DB_FILE, check_same_thread=False)

@lru_cache(maxsize=128)
def query_synergies_cached(sets_key, min_score, max_score):
    ...
    for a, b, score in rows:
        ...
        if count<20:
            print(f"coords[{a}]: {umap_coords[a]}, coords[{b}]: {umap_coords[b]}")
        edges.append({
            "data": {
                "id": f"{a}_{b}",
                "source": str(a),
...
def parse_sets_param(sets_param):
    if isinstance(sets_param, list):
        return [s for s in sets_param if s in all_sets]
    elif isinstance(sets_param, str):
        s = sets_param.strip()
        return [s] if s in all_sets else []
    else:
        return []
...
def graph_data():
    ...
    nodes, edges = query_synergies_cached(sets_key, min_score, max_score)
